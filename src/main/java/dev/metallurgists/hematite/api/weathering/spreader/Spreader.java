package dev.metallurgists.hematite.api.weathering.spreader;

import com.mojang.datafixers.util.Pair;
import dev.metallurgists.hematite.common.weathering.operator.SpreaderOperator;
...
public interface Spreader {
    ...
    Holder<? extends Spreader> getType();
    ...
    default WeatheringAgent getLowInfluenceWeatheringEffect(BlockState state, Level level, BlockPos pos, int maxRecursion) {
        if (maxRecursion > 0) {
            for (SpreaderOperator operator : SpreaderOperator.getAll()) {
                if (!operator.canTarget(state, pos, level)) continue;
                if (operator.isWeathering(state)) {
                    var spreader = operator.getPatchSpreader(this.getType()).orElse(null);
                    if (spreader != null && spreader.getWantedWeatheringState(false, pos, level, maxRecursion - 1)) {
