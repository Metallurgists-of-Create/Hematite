...
public interface NeighborUpdateOperator extends WeatheringOperator {
    ...
    static Set<NeighborUpdateOperator> getAll() {
        return HematiteRegistries.WEATHERING_OPERATOR_REGISTRY.stream().map(op -> op instanceof NeighborUpdateOperator operator ? operator : null).filter(Objects::nonNull).collect(Collectors.toSet());
    }
}
